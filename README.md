...
$env:APP_TIMEZONE="Asia/Qyzylorda"
```
...
Сервер, на котором запущен PHP, должен иметь исходящий доступ к SMTP-серверу
(для Gmail обычно `smtp.gmail.com:587`). Заявка подтверждается только после
успешной отправки временного пароля: если SMTP недоступен, статус заявки
останется «Ожидает подтверждения», и администратор сможет повторить попытку.
...
Если настройки верны, но появляется сообщение о невозможности подключения к
SMTP-серверу, проверьте брандмауэр и наличие исходящего доступа к SMTP-порту у
самого процесса PHP.
