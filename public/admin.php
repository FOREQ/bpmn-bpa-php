...
<section style="position: relative; overflow: hidden;">
    <div class="decor-hex" style="top: 130px; right: calc(50% - 690px); width: 56px; height: 49px;">
        <div class="decor-hex-outer"></div>
        <div class="decor-hex-inner"></div>
    </div>
    <div class="decor-dots" style="bottom: 40px; left: calc(50% - 690px); width: 48px; height: 48px;"></div>

    <div class="container container-wide">
        <div class="admin-header-row">
            <div>
                <div class="admin-eyebrow">Администрирование</div>
                <h1 class="admin-title">Результаты участников</h1>

                <p class="admin-course-line">
                    Курс: «<?= htmlspecialchars($courseTitle) ?>»
                </p>
            </div>

            <div class="admin-actions">
                <a class="btn btn-primary" href="../api/admin/export.php">Скачать CSV</a>
                <button class="btn btn-secondary" id="refreshBtn" type="button">Обновить список</button>
                <a class="btn btn-secondary" href="logout.php">Выйти из админки</a>
            </div>
        </div>
...
<div id="modal" class="modal">
    <div class="modal-content">
        <div class="admin-modal-top">
            <div>
                <div class="admin-eyebrow">Практическая работа</div>
...
<script>
    ...
    function accountStatus(row) {
        ...
        if (status === 'approved') {
            return `
                <span class="status passed">Подтверждена</span>
                <div class="admin-participant-note">Доступ до: ${row.tempPasswordExpiresAt ? escapeHtml(row.tempPasswordExpiresAt) : '—'}</div>
            `;
        }

        if (status === 'rejected') {
            return `
                <span class="status failed">Отклонена</span>
                <div class="admin-participant-note">${row.rejectionReason ? escapeHtml(row.rejectionReason) : 'Без причины'}</div>
            `;
        }
    ...
    function overallGrade(percent, practicalScore) {
        ...
        return `
            ...
            <div class="admin-participant-note">${total} / 50</div>
        `;
    }
...
json.participants.forEach(row => {
    ...
    card.innerHTML = `
        <div class="admin-participant-col wide">
        ...
        <div class="admin-participant-col actions">
            <div class="admin-participant-label">Действие</div>
            ${actionButtons(row)}

            <div class="admin-participant-label" style="margin-top: 4px;">Дата</div>
            <div class="admin-participant-value">
                Регистрация:<br>
