...
function i18nSwitcherMarkup(bool $floating = false): string
{
    ...
    $linkStyle = 'display:inline-flex;align-items:center;justify-content:center;'
        ...
        . 'font:700 12px/1 Segoe UI,Noto Sans,Arial,sans-serif;text-decoration:none;';
...
function i18nTransformOutput(string $output): string
{
    ...
    if ($isHtml && preg_match('/<meta[^>]+name=["\']viewport["\']/i', $output) !== 1) {
        $viewportMarkup = '<meta name="viewport" content="width=device-width, initial-scale=1" '
            . 'data-site-viewport>';
        $output = preg_replace('/<head([^>]*)>/i', '<head$1>' . $viewportMarkup, $output, 1) ?? $output;
    }
